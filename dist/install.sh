...
# Also copy overlay.swift for --gui mode
OVERLAY_SRC="$SCRIPT_DIR/overlay.swift"
if [ ! -f "$OVERLAY_SRC" ]; then
    OVERLAY_SRC="./overlay.swift"
fi
...
# Install overlay.swift (for --gui mode)
if [ -f "$OVERLAY_SRC" ]; then
    if $NEED_SUDO; then
        sudo cp "$OVERLAY_SRC" "$INSTALL_DIR/overlay.swift"
    else
        cp "$OVERLAY_SRC" "$INSTALL_DIR/overlay.swift" 2>/dev/null || true
    fi
    echo "[ OK ] overlay.swift installed (enables --gui)"
fi
